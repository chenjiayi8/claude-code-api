    ...
    try:
        ...
        if raw_body:
            try:
                ...
                logger.info(
                    "JSON parsed successfully", data_keys=list(json_data.keys())
                )
        ...
        if request.stream:
            ...
            async for claude_message in claude_process.get_output():
                ...
                logger.info(
                    ...
                    message_preview=(
                        str(claude_message)[:200] if claude_message else "None"
                    ),
                )
            ...
            await session_manager.update_session(
                session_id=claude_session_id, tokens_used=50, cost=0.001
            )
            ...
            logger.info(
                ...
                choices_0_keys=(
                    list(response["choices"][0].keys())
                    if response.get("choices") and len(response["choices"]) > 0
                    else []
                ),
                message_keys=(
                    list(response["choices"][0]["message"].keys())
                ...
                content_length=(
                    len(response["choices"][0]["message"].get("content", ""))
                    if response.get("choices")
                    and len(response["choices"]) > 0
                    and "message" in response["choices"][0]
                    else 0
